...
It is more fun to use Nozzle interactively in your REPL. Start by creating a nozzle to track the word 'love' and print the resulting data to \*out\*:
...
<pre>
nozzle-example> (nozzle/update-nozzle-password "nozzle_tester" "tHeCoRrEcTpAsSwOrD")
...
Your output buffer should now be overflowing with tons of status messages containging the word 'love'. Now let's track something else, the word 'hate' ...

<pre>
nozzle-example> (nozzle/update-nozzle-stream "nozzle_tester" {:track "love,hate"})
...
<pre>
nozzle-example> (nozzle/update-nozzle-stream "nozzle_tester" {:track "mbta"})
...
<pre>
nozzle-example> (nozzle/update-nozzle-stream "nozzle_tester" {:track "mbta"} "sample") ;; use the sample method
...
<pre>
nozzle-example> (nozzle/update-nozzle-stream "nozzle_tester" {})
...
You could clone this project or use "Leiningen":http://github.com/technomancy/leiningen/ by adding

<pre>[com.ashafa/nozzle "1.0.0-SNAPSHOT"]</pre>

... to dependencies of your project.clj file. 

h3. Todos

* Figure out how to go about writing tests.
* API could use some more love.
* pprint the API responses.
