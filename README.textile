...
<pre>
 (ns nozzle-example
  (:require [com.ashafa.nozzle :as nozzle]
            [com.ashafa.clutch :as clutch] ;; shameless plug
...
(defn start
  ...
  (if (nozzle/get-nozzle-status (auth :id))
    ...
    (nozzle/create-nozzle "filter" (auth :id) (auth :pass) inject-into-couchdb {:track "clojure"})))
...
And there you have it. Streaming twitter with Nozzle. You can save the stauses to a database, check the occurence rate of a words you are tracking, or run all kinds of analysis (such as Natural Language Processing etc.) on the results you get from the streaming API because you now have the power of Clojure, all in real time. I hope you have fun with this.
